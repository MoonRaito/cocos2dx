...
Scene *MapTest01::createScene()
{
    Scene *scene = Scene::create();
    MapTest01 *layer = MapTest01::create();
...
bool MapTest01::init()
{
    ...
    //    listener->onTouchesBegan = CC_CALLBACK_2(MapTest01::onTouchesBegan,this);
    
    // 触摸开始时触发
    //    gameListener->onTouchMoved = [this](Touch* touch,Event* event){return true;};
    listener->onTouchesMoved = CC_CALLBACK_2(MapTest01::onTouchesMoved,this);
...
//    bgSprite->addChild(map, 1);
    ...
    map = TMXTiledMap::create("map04_1.tmx");
    map->setAnchorPoint(Vec2::ZERO);
    map->setPosition(Vec2::ZERO);
    auto scale = 2;
    map->setScale(scale);
    ...
    player01 = Sprite::create("player01.png");
    player01->setPosition(x,y);
    // 将玩家精灵添加为地图的子节点
    map->addChild(player01,3);
    /*******添加人物角色 end**********/
...
void MapTest01::onTouchesMoved(const std::vector<Touch*>& touches, Event *event)
{
    // 获取 大小
        ...
        // 多点进行缩放
    {
        ...
        // 计算map的锚点
        auto anchorX = relMidx / map->getBoundingBox().size.width;
        auto anchorY = relMidy / map->getBoundingBox().size.height;
        ...
        // 当map快要进入到屏幕时，修改bgSprite的位置（既absMidx和absMidy）。
        if( bgOrigin.x > 0)
        {
            absMidx -= bgOrigin.x;
        }
        
        if( bgOrigin.x < -map->getBoundingBox().size.width + winSize.width )
        {
            absMidx += -map->getBoundingBox().size.width + winSize.width - bgOrigin.x;
        }
        if( bgOrigin.y > 0 )
        {
            absMidy -= bgOrigin.y;
        }
        if( bgOrigin.y < -map->getBoundingBox().size.height + winSize.height )
        {
            absMidy += -map->getBoundingBox().size.height + winSize.height - bgOrigin.y;
        }
        
        // 重设map锚点和位置
        map->setAnchorPoint(Vec2(anchorX, anchorY));
        map->setPosition(Vec2(absMidx, absMidy));
        
        // 根据两触摸点前后的距离计算缩放倍率
        auto scale = map->getScale() * ( currDistance / prevDistance);
        ...
        // 更新原点位置
        bgOrigin = Vec2(absMidx, absMidy) - Vec2(map->getBoundingBox().size.width * anchorX, map->getBoundingBox().size.height * anchorY) ;
...
//        
        ...
        // 单点进行移动
    {
        ...
        // 得到此刻map的尺寸
        auto mapCurrSize = map->getBoundingBox().size;
        
        //边界控制，约束pos的位置
        pos.x = MIN(pos.x, mapCurrSize.width * map->getAnchorPoint().x);
        pos.x = MAX(pos.x, -mapCurrSize.width + winSize.width + mapCurrSize.width * map->getAnchorPoint().x);
        pos.y = MIN(pos.y, mapCurrSize.height * map->getAnchorPoint().y);
        pos.y = MAX(pos.y, -mapCurrSize.height + winSize.height + mapCurrSize.height * map->getAnchorPoint().y);
        
        // 重设map位置
        map->setPosition(pos);
        
        // 更新原点位置
        if( pos.x >= mapCurrSize.width * map->getAnchorPoint().x
           || pos.x <= -mapCurrSize.width + winSize.width + mapCurrSize.width * map->getAnchorPoint().x)
        {
            diff.x = 0;
        }
        if( pos.y >= mapCurrSize.height * map->getAnchorPoint().y
           || pos.y <= -mapCurrSize.height + winSize.height + mapCurrSize.height * map->getAnchorPoint().y)
        {
            diff.y = 0;
        }
        bgOrigin += diff;
